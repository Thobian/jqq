...
public class AndroidQQ {
     ...
     long HEART_INTERVAL = 8*60;////心跳时间间隔 如果在手机QQ上注销/退出帐号后，一般10分钟左右您的QQ号就不会显示在线了
     ...
     String keyId = "";//random.randint(0, len(Keys.pubKeys)-1)
	...
	private String verifyToken2;
     ...
     private void unpackRecvLoginMessage(String data) {
    	 /* data = TEA.detea_hexstr(data, this.defaultKey);
    	  HexPacket pack = new HexPacket(null);
    		        String head = pack.shr(Coder.hexstr2num(pack.shr(4))-4);
    		        String body = pack.remain(1);
    		        //head
    		        pack =new HexPacket(null);
    		        ...
    		        pack =new HexPacket(null);
    		        pack.shr(4 + 1 + 2 + 10 + 2);
    		       //int retCode = Coder.hexstr2num(pack.shr(1));
    		        int retCode = 0;
    		        if(retCode == 0){ //登录成功
    		            this.unpackRecvLoginSucceedMessage(pack.remain());
    		           // print u'登录成功: ', this.nickname
    		            System.out.println("登陸成功");
    		            this.alive = true;
    		            this.verify = false;
    		            else if(retCode == 2){ //需要验证码
    		            this.unpackRecvLoginVerifyMessage();
    		            //print this.verifyReason
    		            System.out.println("verifyReason");
    		            this.alive = false;
    		            this.verify = true;
    		            //threading.Thread(target=Img.showFromHexstr, args=(this.verifyPicHexstr, )).start()
    		           // code = raw_input(u'请输入验证码：');
    		            //this.login(Coder.str2hexstr(null));
    		        else{ //登录失败
    		            pack = HexPacket(TEA.detea_hexstr(null, this.shareKey));
    		            pack.shr(2 + 1 + 4 + 2);
    		            pack.shr(4); //type
    		            String title = Coder.hexstr2str(pack.shr(Coder.hexstr2num(pack.shr(2))));
    		            String msg = Coder.hexstr2str(pack.shr(Coder.hexstr2num(pack.shr(2))));
    		            //print title, ': ', msg
    		            this.alive = false;
    		            this.verify = false;
    		        }   */
	...
	public  String  packSendLoginMessage(String verifyCode){
     ...
     public String packSendLoginTlv(String verifyCode){
         if (verifyCode == null){
             ...
             tlv += Tlv.tlv100();
             tlv += Tlv.tlv107();
             tlv += Tlv.tlv144(this.tgtKey, this.imei, this.os_type, this.os_version, this.network_type, this.sim_operator_name, this.apn, this.device, this.device_product);
